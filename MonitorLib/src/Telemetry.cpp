			...
			{
				{
					...
					if (!this->ClientId.empty() && !bQueueEmpty)
					{
						...
						{
#ifdef _LOGGING_ENABLED
							printf("Could not push event data after multiple tries. Network may be down, or server is not available.\n");
#endif
...
bool Telemetry::SendDataToServer()
{
    ...
    // 1) Drain a batch from the queue
    ...
    size_t approxBytes = 2; // [] brackets
    {
        ...
        while (!eventQueue.empty() && batch.size() < MAX_BATCH) 
        {
            ...
            // Rough size estimate: serialize once to gauge limit
            nlohmann::json j = ev;
            ...
            // Accept the event into the batch
            batch.push_back(ev);
    ...
    auto post_json = [&](const std::string& body, bool batched) -> bool 
    {
        ...
        // Consider success if any header line contains '200 OK'
        bool ok = std::any_of(
