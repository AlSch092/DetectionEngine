// IDetector.hpp 
//By Alsch092 @ Github
#pragma once
#include "DetectionResult.hpp"
#include <atomic>

enum ScanIds
{
	Uninitialized = 0,
	ByteSignature,
	ProcessHandles,
	FileHash,
	ManualMap,
	Hypervisor,
	ProcessIsAdmin,
	ProcessString,
	SecureBoot,
	DriverSignatureEnforcement,
	HVCI,
	IATModified,
	UnsignedLoadedModule,
	ProcessCommandLine,
	ResourceUsage, //GPU, memory usage 
	NetworkConnection,
	LoadedDrivers,
	ProcessEnumerator,

	Custom = 100, //user-supplied rule, use a number greater than 100 in your own
};

struct PROCESS_DATA
{
	std::wstring baseName;
	std::wstring path; //full path

	uint32_t pid = 0;

	uint32_t fileHash = 0; //crc32 of file on disk
	uint64_t fileSize = 0;

	bool bIsSigned = false;
	bool bIsWoW64 = false;
	bool bCanObtainHandle = false;

	uint32_t GPUAllocation = 0;
	uint32_t MemoryAllocation = 0;

	//std::vector<MODULE_DATA> loadedModules; //will add a bunch of data, especially if we store all running processes

	std::vector<uint32_t> FlaggedByScans;

	bool operator ==(const PROCESS_DATA& other) const noexcept
	{
		return (path == other.path && pid == other.pid);
	}
};

struct DetectionRule
{
	bool b_Enabled = false; //_ to separate from IDetector::bEnabled

	ScanIds id = Uninitialized; //dictates the type of class which will be created based on the rule (BytePatterScan, OpenHandleScan, etc)

	std::string Label;

	uint8_t Severity = 1; //1 = Info, 2 = Warning, 3 = Critical

	bool bIsProcessScan = false;
	bool bIsOneTimeScan = false;
	bool bIsSelfProcessScan = false;
	bool bIsSystemConfigScan = false; //scans such as secure boot, DSE check, HVCI, etc.
	bool bForceShutdownOnFlag = false;
	bool bTerminateOffendingProcess = false;
	bool bContinuousScan = false;

	std::vector<std::string> Names;
	std::vector<std::string> Artifacts; //process names, byte signatures, etc

	bool operator ==(const DetectionRule& other) const noexcept
	{
		return  this->id == other.id;
	}
};

class IDetector
{
private:
	std::atomic<bool> bEnabled = false; //making DetectionRule.b_Enabled atomic causes issues since nholmann doesn't support it, and overloading copy ctor causes a chain of errors
	DetectionRule RuleData;

protected:
	uint32_t ProcessId = 0;
	std::atomic<uint32_t> RunCount = 0;
	uint8_t LastError = 0;

public:
	IDetector() = default;
	virtual ~IDetector() = default;
	virtual DetectionResult Run() = 0;

	bool IsOneTimeCheck() noexcept { return this->RuleData.bIsOneTimeScan; }
	bool DidOneTimeCheck() const noexcept { return RunCount > 0; }

	bool IsSelfScan() const noexcept { return this->RuleData.bIsSelfProcessScan; }
	bool IsProcessScan() const noexcept { return this->RuleData.bIsProcessScan; }
	bool ShouldShutdownOnFlag() const noexcept { return this->RuleData.bForceShutdownOnFlag; }

	ScanIds GetId() const noexcept { return this->RuleData.id; }
	void SetId(const ScanIds Id) noexcept { this->RuleData.id = Id; }

	void SetIsProcessScan(const bool IsProcScan) noexcept { this->RuleData.bIsProcessScan = IsProcScan; }

	bool IsSystemConfigScan() const noexcept { return this->RuleData.bIsSystemConfigScan; }
	void SetIsSystemConfigScan(const bool IsSystemConfigScan) noexcept { this->RuleData.bIsSystemConfigScan = IsSystemConfigScan; }

	bool IsContinuousScan() const noexcept { return this->RuleData.bContinuousScan; }

	bool ShouldTerminateOffendingProcess() const noexcept { return this->RuleData.bTerminateOffendingProcess; }

	void Deserialize(const DetectionRule& rule)
	{
		this->SetEnabled(rule.b_Enabled);
		this->SetId(rule.id);
		this->SetLabel(rule.Label);
		this->SetSeverity(rule.Severity);
		this->SetIsProcessScan(rule.bIsProcessScan);
		this->SetSelfScan(rule.bIsSelfProcessScan);
		this->SetIsProcessScan(rule.bIsProcessScan);
		this->SetOneShotCheck(rule.bIsOneTimeScan);
		this->SetIsSystemConfigScan(rule.bIsSystemConfigScan);
		this->SetShutdownOnFlag(rule.bForceShutdownOnFlag);
		this->SetTerminateOffendingProcess(rule.bTerminateOffendingProcess);
		this->SetContinuousScan(rule.bContinuousScan);
	}

	bool Enabled() const noexcept
	{
		return this->bEnabled.load(std::memory_order_relaxed);
	}

	void SetEnabled(const bool enabled) noexcept
	{
		this->bEnabled.store(enabled, std::memory_order_relaxed);
	}

	void SetTargetProcess(const uint32_t pid) noexcept
	{
		if (pid == 0 || pid == 4)
			return;

		this->ProcessId = pid;
	}

	void SetSeverity(const uint8_t Severity) noexcept
	{
		this->RuleData.Severity = Severity;
	}

	void SetLabel(const std::string& label) noexcept
	{
		if (!label.empty())
			this->RuleData.Label = label;
	}

	void SetOneShotCheck(const bool isOneTimeChecked) noexcept
	{
		this->RuleData.bIsOneTimeScan = isOneTimeChecked;
	}

	void SetLastError(const uint8_t err) noexcept
	{
		this->LastError = err;
	}

	void SetSelfScan(const bool isSelfScan) noexcept
	{
		this->RuleData.bIsSelfProcessScan = isSelfScan;
	}

	void SetShutdownOnFlag(const bool bShutdownOnFlag) noexcept
	{
		this->RuleData.bForceShutdownOnFlag = bShutdownOnFlag;
	}

	void SetTerminateOffendingProcess(const bool bTerminate) noexcept
	{
		this->RuleData.bTerminateOffendingProcess = bTerminate;
	}

	void SetContinuousScan(const bool bIsContinuous) noexcept
	{
		this->RuleData.bContinuousScan = bIsContinuous;
	}

};